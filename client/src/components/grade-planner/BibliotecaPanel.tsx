...
interface BibliotecaPanelProps {
   ...
   * Sobe o registro completo de tournament_library ao caller quando o jogador
   * CLICA num card (o arrasto segue por conta do rbd). OPCIONAL: sem a prop os
   * cards continuam apenas arrastaveis — back-compat com BibliotecaEmbedded.
   */
  onTournamentClick?: (tournament: any) => void;
}
...
export function BibliotecaPanel({
  ...
  // RF-01: "Importar" + Lixeira recolhidos por default para devolver altura util
  // a lista. Estado transiente de proposito (nao persiste).
  ...
  if (collapsed) {
    return (
      <div
        ...
        className="w-full h-full max-h-full bg-gray-900 border border-gray-700 rounded-lg flex flex-col overflow-hidden"
      >
        {/* Header */}
        ...
        <Droppable droppableId="library" isDropDisabled={true}>
          {(provided) => (
            <div
              ...
              className="flex-1 min-h-32 overflow-y-auto p-1 space-y-0.5"
            >
              {filtered.length === 0 ? (
                  ...
                  <Draggable key={t.id} draggableId={`library-${t.id}`} index={index}>
                    {(dragProvided) => (
                      <LibraryCard
                        ...
                        onCardClick={
                          onTournamentClick ? () => onTournamentClick(t) : undefined
                        }
                      />
                    )}
                  </Draggable>
        ...
        {/* Footer counter — fora do container que rola (RF-01). */}
        <div
          data-testid="library-count"
          className="p-2 border-t border-gray-700 text-center text-xs text-gray-400"
        >
  ...
  // =========================================================================
  return (
    // RF-01: a altura maxima e o sticky sao aplicados pelo caller no Panel
    // (ADR-245 §D7, offset unico). Aqui garantimos que o painel nunca passa da
    // altura que recebeu e que so a lista rola por dentro.
    <div
      data-testid="library-panel"
      className="w-full bg-gray-900 border border-gray-700 rounded-lg flex flex-col overflow-hidden h-full max-h-full"
    >
      {/* Header */}
      ...
      {/* Importar + Lixeira — recolhidos por default (RF-01): com filtros
          avancados abertos a casca do painel comia a altura toda e a lista
          ficava com zero pixel. */}
      <div className="border-b border-gray-700 shrink-0">
        <button
          ...
          className="flex w-full items-center gap-1.5 px-3 py-1.5 text-xs text-gray-400 hover:text-emerald-400 transition-colors"
        >
          {showImportSection ? (
          ...
          Importar e lixeira
          {trashCount > 0 && (
            <span className="ml-auto text-gray-500">Lixeira ({trashCount})</span>
          )}
        </button>
      ...
      {/* Main list or trash view — UNICO scroll container vertical do painel
          (invariante do react-beautiful-dnd, regressao a6b2925c). */}
      {showTrash ? (
        <div
          data-testid="library-scroll"
          className="flex-1 min-h-32 overflow-y-auto p-2 space-y-2"
        >
          {trashCount === 0 ? (
            <div className="text-center text-gray-500 text-xs py-6">
      ...
      {/* Footer counter — sempre visivel, fora do container que rola (RF-01). */}
      <div
        data-testid="library-count"
        className="p-2 border-t border-gray-700 text-center text-xs text-gray-400 shrink-0"
      >
        {filtered.length} de {totalCount} torneios
      </div>
