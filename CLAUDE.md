...
- **AI-1C (Daily Debrief + Monthly Report + tier gating + tool batching + follow-ups):** **95% shipped** em 2026-05-20 — `server/coach/reportEligibility.ts` (`getReportTier`/`isReportEligible` canonicos), `dailyDebriefGenerator.ts` (deterministic, plugado em `handleUpdateGrindSession`), `monthlyReportGenerator.ts` (deterministic + PrimeDope `getVarianceVsExpected` quando disponivel + heuristica fallback), `bulk_query_dimensions` tool, `followUp` block em ambos generators + bloco "Follow-ups abertos" no contexto dinamico do chat. **Migration `0068_report_preferences_daily_monthly.sql` PENDENTE** (`db:push` bloqueado pelo classifier AFK — founder roda manual). **Faltam:** sumarizacao hierarquica Haiku→Sonnet (RF-07 — exige LLM narrative primeiro), LLM narrative em daily/monthly (deterministic-only hoje), Preferences UI toggles (gap herdado AI-1B — nenhum toggle existe nem pro weekly).
