...
- **AI-1C (Daily Debrief + Monthly Report + tier gating + tool batching + follow-ups + LLM narrative + sumarizacao hierarquica):** **TODAS as 8 RFs shipped** em 2026-05-20 — `server/coach/reportEligibility.ts` (`getReportTier`/`isReportEligible` canonicos), `dailyDebriefGenerator.ts` (Sonnet 4.6 narrative + fallback deterministic + plugado em `handleUpdateGrindSession`), `monthlyReportGenerator.ts` (Sonnet 4.6 + PrimeDope variance + heuristica fallback + sumarizacao hierarquica Haiku via `reportSummarizer.ts` quando bundle > 20K chars), `bulk_query_dimensions` tool (RF-06), `followUp` block + bloco "Follow-ups abertos" no contexto dinamico do chat (RF-08), prompts em `server/coach/prompts/dailyDebrief.ts` + `monthlyReport.ts`. **Migration `0068_report_preferences_daily_monthly.sql` PENDENTE** (`db:push` bloqueado pelo classifier AFK — founder roda manual). **Restam apenas:** Preferences UI toggles (gap herdado AI-1B — nenhum toggle existe nem pro weekly; users hoje so mexem via `PUT /api/coach/preferences`).
