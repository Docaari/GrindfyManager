...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post('/api/upload-with-duplicates', requireAuth, upload.single('file'), async (req: any, res) => {
    try {
      ...
      const userId = req.user.userPlatformId;
      const { duplicateAction, duplicateIds } = req.body;
      
      console.log('🔍 Duplicate action:', duplicateAction);
      console.log('🔍 Duplicate IDs:', duplicateIds);
      
      // Validate duplicate action
      if (!['import_new_only', 'import_all', 'skip_upload'].includes(duplicateAction)) {
        return res.status(400).json({ message: 'Ação de duplicata inválida' });
      }
      
      // If skip_upload, just return success
      if (duplicateAction === 'skip_upload') {
        console.log('⏭️ Upload skipped by user');
        return res.json({ 
      ...
      if (isBodogFormat(filename)) {
        parsedTournaments = await PokerCSVParser.parseBodogXLSX(fileBuffer, userId);
      } else if (isCoinFormat(fileContent)) {
        parsedTournaments = await PokerCSVParser.parseCoinTXT(fileContent, userId);
      } else if (isCoinPokerFormat(fileContent)) {
        parsedTournaments = await PokerCSVParser.parseCoinPokerCSV(fileContent, userId);
      } else {
        parsedTournaments = await PokerCSVParser.parseCSV(fileContent, userId);
      }
      
      console.log('✅ Parsed tournaments:', parsedTournaments.length);
      
      // Filter tournaments based on duplicate action
      let tournamentsToImport = parsedTournaments;
      
      if (duplicateAction === 'import_new_only' && duplicateIds) {
        // Parse duplicateIds if it's a string
        const duplicateIdArray = typeof duplicateIds === 'string' ? JSON.parse(duplicateIds) : duplicateIds;
        
        // Filter out tournaments that are duplicates
        tournamentsToImport = parsedTournaments.filter(tournament => {
          const isDuplicate = duplicateIdArray.some((dupId: string) => 
            tournament.tournamentId === dupId || tournament.name === dupId
          );
          return !isDuplicate;
        });
        
        console.log('🔄 Filtered to new tournaments only:', tournamentsToImport.length);
      }
      // For 'import_all', we keep all tournaments
      ...
      const uploadHistoryData = {
        ...
        tournamentsCount: tournamentsToImport.length,
        uploadDate: new Date(),
        duplicatesFound: parsedTournaments.length - tournamentsToImport.length,
        duplicateAction
      };
      
      if (tournamentsToImport.length > 0) {
      ...
      await storage.createUploadHistory(uploadHistoryData);
      
      console.log('✅ Upload completed:', {
        imported: importedCount,
        duplicates: parsedTournaments.length - tournamentsToImport.length,
      ...
      // Create failed upload history entry
      const uploadHistoryData = {
        id: nanoid(),
        userId: req.user.userPlatformId,
        ...
        duplicatesFound: 0,
        duplicateAction: req.body.duplicateAction || 'unknown'
      };
