...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post('/api/weekly-routine/generate', isAuthenticated, async (req: any, res) => {
    try {
      ...
      // Limpar eventos gerados automaticamente da semana anterior
      const weekStartDate = new Date(weekStart);
      const weekEndDate = new Date(weekStart);
      weekEndDate.setDate(weekEndDate.getDate() + 6);
      
      const existingEvents = await storage.getCalendarEvents(userId, weekStartDate, weekEndDate);
      const autoGeneratedEvents = existingEvents.filter(event => event.source === 'intelligent_routine');
      
      for (const event of autoGeneratedEvents) {
