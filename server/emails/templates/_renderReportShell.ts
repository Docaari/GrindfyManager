...
// Cada template injeta apenas o miolo (`bodyHtml`); shell cuida do header
// (saudacao escapada), CTA e footer (disclaimer + unsubscribe).
//
// API:
//   renderReportShell({userName, subject, intro, bodyHtml, ctaLabel, ctaUrl,
//                      disclaimer?, unsubscribeUrl}): { html, text }
//
// Lesson #16: DOMPurify-safe — userName escapado (XSS). bodyHtml e' interpolado
// literal (eh HTML ja pronto vindo do template).
// =============================================================================
...
export function renderReportShell(args: RenderReportShellArgs): RenderReportShellResult {
  ...
  const bodyText = stripTags(args.bodyHtml || "");
  const textLines = [
    `Olá ${args.userName},`,
    "",
    args.intro,
