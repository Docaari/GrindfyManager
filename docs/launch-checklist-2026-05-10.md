...
- [~] **Em andamento (item separado, baixa prioridade):** client project (jsdom). 2026-05-11: cluster `SessionSummaryModal.*.test.tsx` limpo (cooldown CTA removido em 2363509 — 2 files deletados, 4 reescritos pra `cta-finalize-session`) + `coach-context-assembly` ajustado pro fix do coachContext → ~102 → **~69 red**. Restante: `CoachAI.*.test.tsx` (~25 — pagina refatorada no sprint coach-page-reform-1), `Biblioteca-2`/`FxRatesPanel` (~41 — lesson #14/#26: `require()` em `.tsx`, fix = test-writer converter pra `await import()`; nem sempre funciona pelo resolver embebido do vitest), `ProtectedRoute`/`MessageCopyButton`/`StatsAnalyzerTab`/`GrindSessionLive.spot-paster` (~9). NAO bloqueia deploy (Docker build nao roda testes).
