...
- [~] **Em andamento (item separado, baixa prioridade):** client project (jsdom). Sweep 2026-05-11/12 → ~102 → **25 red, e os 25 sao TODOS `CoachAI.*.test.tsx`** (pagina em refatoracao na sessao paralela do founder — assertions de UI divergem do estado atual; nao tocar daqui). Cleared: `SessionSummaryModal.*` (cooldown CTA removido em 2363509 — 2 files del, 4 reescritos), `GrindSessionLive.spot-paster` (wouter mock +useSearch), `MessageCopyButton` (clipboard via defineProperty + fireEvent), `ProtectedRoute` (gate admin virou prefixo `/admin*` — **fechou gap: /admin/dashboard estava acessivel a non-admin**), `StatsAnalyzerTab` (snapshots viraram Select), `Home.minichat-smoke` (import lazy aceito), `flight-redirect` (memoryLocation `history` em vez de `hook()`), `FlightPage` (item virou /coach?tab=flights), `FxRatesPanel` (require()→await import(), lesson #14/#26), `TicketsWidget` (NOW=Date.now()), `LessonRow` ("Acesso restrito"), `LessonViewer.videoSpeed` (mock playback-token), `AlertsPanel.tts`+`GrindSessionLive.tts-wiring` (expandir Collapsible). NAO bloqueia deploy (Docker build nao roda testes).
