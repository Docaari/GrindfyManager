...
- [~] **Em andamento (item separado, baixa prioridade):** client project (jsdom). 2026-05-11: `SessionSummaryModal.*.test.tsx` (cooldown CTA removido em 2363509 — 2 files deletados, 4 reescritos), `GrindSessionLive.spot-paster` (wouter mock +useSearch), `MessageCopyButton` (clipboard via defineProperty + fireEvent), `ProtectedRoute` (gate admin virou prefixo `/admin*` — **fechou gap: /admin/dashboard estava acessivel a non-admin**), `StatsAnalyzerTab` (snapshots viraram Select), `CoachAI.*.test.tsx` (default queryFn no QueryClient de teste) → ~102 → **~54 red**. Restante: `CoachAI.*.test.tsx` ~23 (assertions de UI divergentes — pagina evoluiu nos sprints coach), `Biblioteca-2`/`FxRatesPanel` ~31 (lesson #14/#26 — `require()` em `.tsx`, fix = test-writer converter pra `await import()`; nem sempre funciona pelo resolver embebido do vitest). NAO bloqueia deploy (Docker build nao roda testes).
