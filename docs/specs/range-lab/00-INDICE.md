...
| [F3b Aritmetica da decisao](F3b-decisao.md) | Cascata, bloqueadores, MDF / value-blefe | Opus 5 — Alto | F1 (nao depende da F3a) | **Concluida** 2026-08-18. `mdf.ts` + `confront.ts` + `cascade.ts` + `blockers.ts` (nucleo puro) e `MdfPanel`/`CascadeBar`/`BlockerPanel` em `/range-lab`. 970 verdes na area (818 antes + 152 novos), `tsc` 0. [ADR-249](../../architecture/decisions/249-range-lab-f3b-decisao.md) com **tres refutacoes** que mudaram o desenho (combos bloqueados JA vem em `perVillainCombo` com `pairMass 0`; o degrau 4 nao e `sum(pairMass)`; a invariante testavel e no degrau 3) + D-F3-23..D-F3-38 |
...
| D-F3-23 (2026-08-18, F3b) | Forma do retorno do MDF | `mdf.ts` devolve **uniao discriminada** (`ok`/`degraded`), nao objeto de campos nulos. Campo lido e campo que alguem le: um `mdf: null` sobrevive a um `if` esquecido e vira `NaN%` na tela; campo AUSENTE nao tem como vazar. Mesmo principio da D8 e da D12 |
| D-F3-24 (2026-08-18, F3b) | Onde mora value/blefe/chop | Terceiro modulo, `confront.ts`. Dois paineis precisam da mesma conta (MDF e bloqueador); em `mdf.ts` faria `blockers.ts` importar o alpha, em `blockers.ts` faria `mdf.ts` importar o modulo pesado. Arvore rasa e aciclica |
| D-F3-25 (2026-08-18, F3b) | De onde sai a base declarada | Do RESULTADO, nunca do estado da UI. `discrete` exige river **E** heroi de um combo so — no river com heroi como range a equity volta a ser continua, e uma contagem de "combos que perdem" seria inventada (o mesmo erro que a D11 documentou). O predicado le `perHeroCombo.length`, nao o toggle "Minha mao": com ele ligado o jogador ainda pode pintar `AKs` e ter 4 combos |
| D-F3-26 (2026-08-18, F3b) | Onde vive a invariante da cascata | No **degrau 3**, nao no 5. O bloco de maos feitas da F3a soma o range apos o card removal do bordo, que e exatamente o degrau 3; o degrau 5 e subconjunto do 4, que e subconjunto do 3 — se batesse com a F3a, um dos dois estaria errado. A queda do 3 para o 4 e o que a cascata existe para mostrar. Degrau sem dado sai `unavailable` com razao, **nunca** com massa zero (que na barra seria lido como "suas cartas mataram o range inteiro") |
| D-F3-27 (2026-08-18, F3b) | Contrafactual do bloqueador | **Sem segunda corrida do motor.** Rodar de novo nao serve (o motor volta a excluir os pares por `collides()`) e trocar carta do heroi conflaciona dois efeitos. Os bloqueados ja vem em `perVillainCombo` com `pairMass: 0`; so falta a equity do par ficticio, enumerada em `blockers.ts` com denominador **por combo** — `C(46,2) = 1035` no flop, e nao os `990` de `cost.ts`, que dariam vies sistematico de 4,5% |
| D-F3-28 (2026-08-18, F3b) | Reentrancia do `fastEvaluator` | `loadBoard` e estado de MODULO e agora tem dois clientes. Contrato escrito: quem chama `evalWithBoard` chama `loadBoard` no MESMO bloco sincrono. Por isso `blockers.ts` nao pode ser `async` — ha teste estrutural cobrando |
| D-F3-29 (2026-08-18, F3b) | Delta no Monte Carlo | **Suprimido** com razao (`monte_carlo_mode`). Um delta tipico de bloqueador vive entre 1 e 3 pontos percentuais e a meia-largura do agregado pode ser da mesma ordem: o numero sairia menor que o proprio erro, com uma casa decimal. A contagem de combos removidos continua, porque nao depende da corrida |
| D-F3-35 (2026-08-18, F3b) | `unknown` so em `equity_vs_range` | A regra "combo sem equity nunca vira blefe" vale onde a equity **e** o criterio. Os combos bloqueados tem `equity: null` por construcao; lida ao pe da letra, a regra faria o painel de bloqueadores nascer em branco. Com heroi de mao unica sao as CARTAS que decidem |
| D-F3-36 (2026-08-18, F3b) | A massa sob base efetiva | `degrau5 = SUM w * (1 - equity)` (o chop ja entra pela metade dentro da equity); no ramo discreto, `blefe + 0,5 * chop`, a mesma grandeza. Nos baldes, `mass` e sempre o peso **declarado**: a base decide como o combo e CLASSIFICADO, nao quanto ele pesa |
| D-F3-37 (2026-08-18, F3b) | Combo sem numero no ranking | Fica **fora**. No fim seria dizer que e o pior, no comeco que e o melhor. Desempate por `comboKey` e regra, nao excecao: no river metade do range tem equity 0 ou 1 e a ordem acompanha as entradas, que mudam quando o jogador pinta uma celula — sem desempate total o `k` da tela piscaria sozinho |
