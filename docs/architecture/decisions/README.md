...
| [147](147-coach-read-tool-service-extraction-pattern.md) | Read tools do Coach: extrair `tournamentScoringService` (DRY — nunca duplicar `computeTournamentScore`; `get_tournament_suggestions`/`explain_tournament_score` + route do Selector reusam); fonte canonica de banca p/ `simulate_bankroll_scenario` = `walletService.getConsolidatedBalance` (USD, FX-aware) com fallback interno p/ `user_settings.bankroll_amount`; unificar blocos de prompt citation/confidence (aceita quebra de cache unica, remove variante backticked — lesson #10) (Sprint AI-0A) | Aceito | 2026-05-12 |
