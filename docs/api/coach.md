...
## Sprint AI-1C — Daily Debrief + Monthly Report + tier gating estrito + sumarizacao hierarquica + tool batching + follow-ups (ADR-159/160/161)

**Nenhum endpoint HTTP novo.** Os endpoints do AI-1B passam a servir os tipos `'daily'`/`'monthly'` transparentemente; o Daily Debrief e enfileirado internamente via `handleUpdateGrindSession`; o Monthly via o enqueuer hourly. Migracao `0068` (2 booleanos em `user_coach_preferences`).

### `GET /api/coach/timeline` — estendido (sem mudanca de contrato)
Agora retorna itens `kind:'report'` com `reportType` ∈ {`'weekly'`, `'daily'`, `'monthly'`} ordenados por `generatedAt` desc junto com os nudges. O `ReportsPanel` mostra um **badge do tipo** em cada card ("Diario"/"Semanal"/"Mensal").

### `GET /api/coach/reports/:id` — estendido (sem mudanca de contrato)
Serve **qualquer tipo** de relatorio (`content` = `ReportContent` v2 + `markdown`). Marca `read_at`. **404** inexistente; **403** de outro user. Render: rota `/coach-ai/relatorio/:id` -> **`ReportView.tsx`** (generalizado de `WeeklyReportView`) — `weekly` = as 8 secoes (sem regressao); `monthly` = 5 secoes + evolucao (`comparatives`) + variancia + leaks resolvidos/novos + progresso das metas + "Seu acompanhamento" (`followUp`); `daily` = resumo da sessao (`sessionSummary`) + feedback + "Seu acompanhamento". CTAs idem AI-1B (`kind:'link'` -> rota Wouter registrada; `kind:'tool'` -> fluxo da tool com confirm, NAO auto-executa). `status='degraded'` -> numeros + aviso "modo simplificado".

### `ReportContent` v2 (shape de `reports.content` — interface TS em `shared/schema.ts`, `schemaVersion: 2`)
`reportType` alargado para `"weekly" | "monthly" | "daily"`. Campos opcionais novos (frontend v1 ignora — lesson #7):
- `comparatives?`: `{ previousPeriod?: { label, profit, roi, count }, last6Months?: Array<{ month, profit, roi, count }> (month = 'YYYY-MM'), last12Months?: Array<...>, trendNarrative? }` (monthly).
- `variance?`: `{ bankrollDeltaUsd, estimatedBySkillUsd, estimatedByVarianceUsd, sampleSize, method: 'heuristic'|'primedope', narrative?, confidence? }` (monthly — `'heuristic'` enquanto `getVarianceVsExpected` retorna `null`; `'primedope'` quando AI-2A `analyze_variance`).
- `leaksDelta?`: `{ resolved: Array<{ code, label, note? }>, newSignals: Array<{ code, label, severity? }>, activeFocus: Array<{ code, label, status, progressNote? }> (status via verify_leak_progress), narrative? }` (monthly).
- `goalsProgress?`: `Array<{ goalId, texto, prazo: 'mes'|'trimestre'|null, estimate: 'on_track'|'behind'|'ahead'|'unknown', narrative? }>` (monthly — uma entrada por `AiStructuredProfileMeta`; `'unknown'` quando nao da pra inferir).
- `followUp?`: `{ activeLeakFocus: Array<{ code, label, targetMonth, status, progressNote? }>, goalsInProgress: Array<{ goalId, texto, prazo }>, narrative? }` (weekly/monthly/daily — RF-08 / ADR-161; **ausente quando nao ha foco de leak ativo nem meta** — nao inventa, lesson #11).
- `sessionSummary?`: `{ sessionDate, sessionsCount, durationMinutes, tournaments, itmPct, finalTables, wins, profitByCurrency: Array<{ currency, native, usd }>, roiSession, spotsCount, spotHighlight?, vsRecentNarrative? }` (daily — ou monthly agregando sessoes).
- `sections` continua obrigatorio no shape; o daily preenche minimamente — o `ReportView` nao renderiza bloco vazio.

### `GET /api/coach/preferences` — estendido (ADR-159)
`buildPrefsResponse` ganha **`reportDailyEnabled: boolean`** (default `false`) + **`reportMonthlyEnabled: boolean`** (default `false`) — alem do `reportWeeklyEnabled` do AI-1B. A aba "Preferencias" do hub ganha os 2 toggles novos ("Debrief diario das sessoes" + "Relatorio mensal") — **so visiveis pra tier elegivel** (`getReportTier === 'eligible'`: Trial + Pro/Premium + Admin); Free ve linha desabilitada/nada.

### `PUT /api/coach/preferences` — estendido (ADR-159)
Ganha **`reportDailyEnabled?`** + **`reportMonthlyEnabled?`** (`z.boolean().optional()`; zod `.strict()` mantido — campos crus de congelamento rejeitados -> 400). Mesmo se um Free setar `true` via API direta, o enqueuer/processor revalida `getReportTier` -> nao enfileira / `skipped` (defesa em profundidade).

### `PUT /api/grind-sessions/:id` — estendido (interno, sem mudanca de contrato da resposta)
Quando `req.body.status === 'completed'`, alem de `stopService.evaluateStops`, dispara `void enqueueDailyDebriefForSession({ userId, sessionId: id, now }).catch(console.error)` — **best-effort, fire-and-forget, depois do `res.json()`**: a resposta sai imediatamente e **nao falha** se o enqueue der erro (try/catch — lesson #9). O helper revalida `isReportEligible(userId, 'daily')` + `COACH_NUDGES_ENABLED` + pre-check de cap diario, depois `insertReportJobOnConflictDoNothing({ reportType:'daily', periodStart: <data da sessao no fuso do user>, periodEnd: <mesma data>, scheduledFor: now, status:'pending', timezone, subscriptionPlanAtEnqueue, enqueuedBy:'session_completed' })`. **NAO gera o relatorio sincrono** — o `processReportJobsTick` (15min) gera via `dailyDebriefGenerator`.

### Tier gating estrito (ADR-159 — `server/coach/reportEligibility.ts`)
- `getReportTier(user): Promise<'free' | 'eligible'>` — `'eligible'` quando `user.subscriptionPlan === 'trial'` OU `resolveUserTier(user)` ∈ {`'pro'`, `'premium'`, `'admin'`} (cobre admin via role/plan e `'active'` via JOIN `user_subscriptions`); `'free'` caso contrario. **`resolveUserTier` em si NAO muda** (gateia rate limit + tools — Trial -> free la, mantem).
- `isReportEligible(userId, reportType: 'weekly'|'daily'|'monthly'): Promise<boolean>` — resolve o user (`getUserById` -> `subscriptionPlan`/`role`) + `getReportTier !== 'free'` + `getCoachPreferences(userId).report{Weekly|Daily|Monthly}Enabled === true`. **Safe-deny em erro de leitura** (lesson #9).
- Enqueuer: `listUsersForCron("subscription_plan IN ('trial','active','admin')")` para o candidate set + `isReportEligible` filtra os elegiveis (descarta `'active'` sem subscription = free; confirma pro/premium via `resolveUserTier`). Processor: `revalidateEligibility` usa `getReportTier` (downgrade entre enfileirar e processar -> `status='skipped'`, `last_error='no_longer_eligible'`).
- **Corrige o bug latente do AI-1B:** `PRO_PLANS = ['pro','premium','admin']` + `listUsersForCron("subscription_plan IN ('pro','premium')")` so pegava `admin` (porque `users.subscription_plan` e `'trial'|'active'|'expired'|'admin'`, nunca `'pro'`/`'premium'`). `isWeeklyReportEligible` e refatorado pra delegar a `isReportEligible(userId, 'weekly')`; `PRO_PLANS` removido de `reportJobRunner.ts`.

### Daily Debrief — gerador (ADR-159 — `server/services/dailyDebriefGenerator.ts`)
`generateDailyDebrief({ userId, periodStart, failSoft?, injectedStorage? })` (`periodEnd = periodStart`). Bundle: a(s) sessao(oes) do dia (`getGrindSessions(userId)` filtra por data + `status` ∈ {`'completed'`, `'finished'`}) + `getSessionTournaments(sessionId)` (FX -> USD — lesson #6; reusa `grindSessionHistory.ts` se ele ja agrega) + spots/notas (`starred_hands` da sessao, `capturedDuring='grind-live'`) + ritmo recente (`getGrindSessions` ultimas ~10 ou `getPerformanceByPeriod('30d')`) + foco de leak ativo + metas (`findActiveLeakFocusList` + `getAiStructuredProfile`). Conteudo: `header` + `sessionSummary` + `insights` (1-2 data-grounded com `[fonte:]`) + `followUp` (se houver foco/meta) + `cta` (so tools AI-0A / rotas Wouter registradas). `dataSufficiency='low'` se 0 torneios e 0 spots -> minimalista, sem LLM, custo ~ 0. Fail-soft identico ao AI-1B (`new AnthropicCtor` try/catch — lesson #5/#35; sem `ANTHROPIC_API_KEY` -> `degraded` `degraded_reason='no_anthropic_key'` job `done` 1a tentativa; LLM falha 3x -> `degraded` `'llm_failed_3x'` job `done` nunca `failed`). **NAO** roda `runBackfills`; **NAO** aciona a sumarizacao hierarquica (bundle pequeno — `summarizer_model_used = null`). Modelo: `process.env.COACH_MODEL ?? 'claude-sonnet-4-6'`, `max_tokens ~1200`. Custo ~$0.013. Prompt unico `server/coach/prompts/dailyDebrief.ts` (reusa `GRINDFY_AI_BASE` + `CITATIONS_RULES` + `cache_control: ephemeral` — lesson #10).

### Monthly Report — gatilho + gerador (ADR-159 — `server/services/monthlyReportGenerator.ts`)
- **Gatilho:** regra no enqueuer hourly do AI-1B (`enqueueWeeklyReportJobsTick` — agora enfileira weekly E monthly): para cada user elegivel com `reportMonthlyEnabled=true`, se `getLocalHour(now, tz) === 7 && localCivilDate(now, tz).civilUtc.getUTCDate() === 1` (dia 1 do mes no fuso) -> `period_start` = 1o dia do mes anterior (aritmetica `Date.UTC` — vira do ano OK; `period_end` = ultimo dia do mes anterior, 28/29/30/31), `INSERT ... ON CONFLICT DO NOTHING (report_type='monthly', ..., enqueued_by='cron_enqueuer')`. UNIQUE `(user_id, 'monthly', period_start)` -> 1/mes.
- **Gerador:** `generateMonthlyReport({ userId, periodStart, periodEnd, failSoft?, injectedStorage? })`. Bundle: volume/resultados do mes (`getDashboardStats(userId, 'custom', { dateFrom: periodStart, dateTo: periodEnd })` para os contadores ricos + `getPerformanceByPeriod(userId, periodStart + ' to ' + periodEnd)`) + bankroll do mes (`getConsolidatedBalance` em USD + `getBankrollSnapshots({ to: monthEnd })` + `listWalletTransactionsByUser({ from, to })`) + selection (`getAnalyticsBy*`) + estudos (`getStudySessionsV2`/`V1` + `findActiveLeakFocusList` + `getAiStructuredProfile.focoDoMes` + `recommendLessonForUser`) + mental (`listWarmupRituals({ from, to })` — so warm-up, Q6) + **comparativos** (`buildMonthlyComparatives(12)` compondo `getPerformanceByPeriod` por mes — `getMonthlyPerformanceSeries` NAO criado; serie `[{ month, profit, roi, count }]` x 12, ordenada; `previousPeriod` = mes -1, `last6Months` = ultimos 6, `last12Months` = todos; toda query filtra `grind_session_id IS NULL` — §6.1) + **variancia** (`getVarianceVsExpected` retorna `null` hoje -> heuristica ROI observado vs ROI medio 12m, desvio em buy-ins, `variance.method='heuristic'`, confidence baixo, rotulado como estimativa; `'primedope'` quando AI-2A) + **leaks resolvidos/novos** (`detectLeaks` x `coach_leak_focus` + a logica de `verify_leak_progress` — sem tabela historica nova) + **progresso das metas** (`getAiStructuredProfile.metas` — uma entrada por meta em `goalsProgress`, LLM interpreta o texto). Conteudo: 5 secoes do weekly (com comparativo curto vs mes -1 cada) + `comparatives` + `variance` + `leaksDelta` + `goalsProgress` + `insights` (3-5 data-grounded com `[fonte:]`) + `nextWeekPlan` (foco + acao — NAO monta a grade) + `followUp` + `cta`. **Roda `runBackfills`** (cobre >=1 semana — `coach_lesson_recommendations` + `study_weekly_plans` idempotente). **Aciona a sumarizacao hierarquica** se o bundle exceder o threshold (RF-07 — esp. com 12 meses de serie; `summarizer_model_used` preenchido). `dataSufficiency='low'` se mes vazio (user novo) -> minimalista, header convidando a importar, CTA `/upload`, custo ~ 0. Fail-soft identico (`new AnthropicCtor` sonnet E haiku — lesson #5/#35). Modelo: `process.env.COACH_MODEL ?? 'claude-sonnet-4-6'`, `max_tokens ~4000`. Custo ~$0.11. Prompt unico `server/coach/prompts/monthlyReport.ts`.

### Sumarizacao hierarquica Haiku -> Sonnet (ADR-159 RF-07 — `server/coach/hierarchicalSummarizer.ts`)
`summarizeBundleHierarchical({ bundle, reportType, model? }): Promise<{ summarizedBundle, summarizerModelUsed: string|null, layersUsed: number }>`. Threshold: `JSON.stringify(bundle).length > COACH_REPORT_SUMMARIZE_THRESHOLD_CHARS` (default ~20000) — abaixo -> no-op (`summarizerModelUsed=null`, `layersUsed=0`, nao chama o LLM). Modelo Haiku: `process.env.COACH_REPORT_SUMMARIZER_MODEL ?? process.env.COACH_MEMORY_MODEL ?? 'claude-haiku-4-5-20251001'` (mesma fonte de `coachMemory.ts`). Camadas (para as partes "longas" — esp. `last_12_months`): raw -> 12 resumos/mes (1 chamada Haiku) -> 4 resumos/trimestre (1 chamada) -> 1 resumo/ano (1 chamada); so o resumo final (ano) + os numeros crus das janelas-chave (mes -1, 6m) + o "mes corrente em detalhe" vao pro sonnet. Daily nunca aciona; weekly so se exceder. Fail-soft: Haiku falha (excecao / sem `ANTHROPIC_API_KEY` / `new AnthropicCtor` que e `vi.fn()` — lesson #5/#35) -> loga (antes do fallback — lesson #9) + usa o bundle **truncado** (corta as series pros ultimos N meses, sem LLM), `summarizerModelUsed=null`. Reusa o pattern de `coachMemory.ts` (a chamada base — `getAnthropicClient` + `messages.create` + `getMemoryModel`); so a estrutura de camadas e nova. **Substitui o stub "sumariza com Haiku se grande" do AI-1B** — o `weeklyReportGenerator.ts` refatorado usa o mesmo helper.

### Generalizacao do gerador (ADR-159 RF-04)
Helpers compartilhados em `server/services/reportGeneratorShared.ts` (extraidos de `weeklyReportGenerator.ts` — sem regressao): `DEFAULT_REPORT_MODEL`/`getReportModel()`, `ALLOWED_HREFS`/`sanitizeHref`, `computeCost`, `n`/`ymd`/`safe`/`resolveStorage`, `callLlm` (`new AnthropicCtor` try/catch + fallback factory — lesson #5/#35; `cache_control: ephemeral`), `persistReport` (**`reportType` e parametro** — nao hardcodado `'weekly'`; -> `storage.upsertReport`), `renderMarkdownBase`, `buildCtas`/`mergeLlm`, `buildReportFollowUp` (ADR-161). `weeklyReportGenerator.ts` refatorado + `dailyDebriefGenerator.ts` + `monthlyReportGenerator.ts` reusam. `processReportJobsTick` despacha por `job.reportType` via `getGeneratorFor(reportType)` — `'weekly'`->`generateWeeklyReport`, `'daily'`->`generateDailyDebrief`, `'monthly'`->`generateMonthlyReport`, `'quarterly'`/desconhecido -> `console.warn('report.job.unsupported_report_type')` + `status='skipped'` (nao quebra o tick). `persistOrFetchReportId`/`getReportForPeriod`/`upsertReport` no `reportJobRunner.ts` usam `claimed.reportType` (nao o literal `'weekly'`).

### Follow-ups inteligentes (ADR-161)
- Secao "Seu acompanhamento" (`ReportContent.followUp`) nos 3 tipos — `buildReportFollowUp(storage, userId)` (em `reportGeneratorShared.ts`): foco de leak ativo (`findActiveLeakFocusList` + status via a logica de `verify_leak_progress` — reusada como funcao, nao como tool do LLM) + metas em progresso (`getAiStructuredProfile.metas` com `prazo` ∈ {`'mes'`, `'trimestre'`}) + `focoDoMes`; **se nada ativo -> `followUp` ausente / secao omitida** (lesson #11). O LLM gera a `narrative` (1-2 frases fechando o loop). Render `## Seu acompanhamento` (daily: `Acompanhamento`) no markdown + `ReportView` so quando presente.
- Bloco `## Follow-ups abertos` no contexto **DINAMICO** do chat (`coachContext.ts` `assembleContext` — padrao `systemParts.push`, reusa `findActiveLeakFocusList`/`getAiStructuredProfile` sem duplicar queries, atencao lesson #17; vai no DINAMICO — NAO no STATIC, NAO quebra o cache).
- 1 instrucao no `GRINDFY_AI_BASE` (bloco STATIC, prompt unico do agente — lesson #10): "quando houver um bloco '## Follow-ups abertos' no contexto e for natural na conversa, mencione brevemente o follow-up pendente e ofereca fechar o loop — sem ser repetitivo nem interromper o assunto do usuario".
- Conteudo **passivo** (nao e nudge — nao passa pelo `shouldSendNudge`, nao tem `cycleKey`/quiet hours; risco de "ser chato" mitigado — e secao de um relatorio que o user ja optou em receber).

### Gating (ADR-159)
`COACH_NUDGES_ENABLED === 'false'` desliga o enqueuer (weekly + monthly) + o processor + os ticks de gap-check/B-IMPORT + **o enqueue event-driven do Daily Debrief** em `handleUpdateGrindSession` (`enqueueDailyDebriefForSession` vira no-op — relatorios contam como "proatividade"). Jobs ja enfileirados ficam parados; quando volta, o processor pega os atrasados.

### Referencias AI-1C
- ADR-159: `Docs/architecture/decisions/159-ai-1c-daily-debrief-monthly-report-tier-gating-hierarchical-summarization.md`
- ADR-160: `Docs/architecture/decisions/160-bulk-query-dimensions-tool-batching.md`
- ADR-161: `Docs/architecture/decisions/161-follow-ups-inteligentes-coach.md`
- Diagramas: `Docs/architecture/diagrams/coach-ai-1c/{daily-debrief-flow,monthly-report-flow,report-generator-structure,tier-gating-flow,hierarchical-summarizer-layers,follow-ups-flow}.mermaid`
- Spec: `Docs/specs/sprint-ai-1c.md`
- Tool nova: `bulk_query_dimensions` — ver `Docs/api/coach-tools.md`
